...
/**
 * AnalogClockSwing 의 팝업 메뉴 생성 로직을 분리한 클래스.
 * AnalogClockSwing 인스턴스를 HostContext 인터페이스로 주입받아
 * 부모 클래스에 대한 직접 의존 없이 JPopupMenu 를 빌드한다.
 */
public class MenuBuilder {
    private static final String mainTitle = "■■ 끝판왕 (v1.42)";
    ...
    public JPopupMenu build() {
        ...
        bgItem.addActionListener(e -> {
            ...
            if (c != null) { host.stopSlideTimer(); host.stopShowTimer(); host.stopCamera(); host.setGalaxyMode(false); host.setMatrixMode(false); host.setBgImage("", null); host.setBgColor(c); host.saveConfig(); host.repaintClock(); }
        });
        ...
        bgImageItem.addActionListener(e -> {
            ...
            // 이전에 선택한 파일이 있으면 그 폴더를 기본으로
            String prev = host.getBgImagePath();
            if (prev != null && !prev.isEmpty()) {
                java.io.File prevFile = new java.io.File(prev);
                if (prevFile.getParentFile() != null) fc.setCurrentDirectory(prevFile.getParentFile());
            }
            int result = fc.showOpenDialog(host.getOwnerComponent());
        ...
        bgReset.addActionListener(e -> { host.stopSlideTimer(); host.stopShowTimer(); host.stopCamera(); host.setGalaxyMode(false); host.setMatrixMode(false); host.setBgImage("", null); host.setBgColor(null); host.repaintClock(); });
        JMenuItem bgGalaxy = new JMenuItem("Galaxy");
        bgGalaxy.addActionListener(e -> { host.stopSlideTimer(); host.stopShowTimer(); host.stopCamera(); host.setBgImage("", null); host.setGalaxyMode(true); host.setMatrixMode(false); host.setBgColor(null); host.repaintClock(); });
        JMenuItem bgMatrix = new JMenuItem("Matrix");
        bgMatrix.addActionListener(e -> { host.stopSlideTimer(); host.stopShowTimer(); host.stopCamera(); host.setBgImage("", null); host.setMatrixMode(true); host.setGalaxyMode(false); host.setBgColor(null); host.repaintClock(); });
        // ── 배경색 Show ────────────────────────────────────────
        JMenu showMenu = new JMenu("배경색 Show...");
        for (int v : host.getIntervals()) {
            JMenuItem si = new JMenuItem(v == 0 ? "0 (중단)" : String.valueOf(v));
        ...
        bgMenu.add(showMenu);     // 6 (이동)
        bgMenu.add(new javax.swing.JSeparator());
        JMenuItem speedGuide = new JMenuItem("⚡ 크기 및 속도 조절");
        speedGuide.addActionListener(e -> JOptionPane.showMessageDialog(
        ...
        for (int v : host.getIntervals()) {
            JMenuItem ai = new JMenuItem(v == 0 ? "0 (중단)" : String.valueOf(v));
        ...
        telegramHelpItem.addActionListener(e -> {
            // 기존 showTelegramHelp() 대신 블로그 URL을 바로 브라우저로 열기 (요구사항 3)
            try {
        ...
        // 2번: About - 블로그 URL을 백그라운드에서 열고, 다이얼로그를 앞에 표시
        JMenuItem aboutItem = new JMenuItem("About (Ver 1.41)");
        aboutItem.addActionListener(e -> {
            // 브라우저는 별도 스레드로 열어 다이얼로그 포커스를 빼앗기지 않게 함
            new Thread(() -> {
                try {
                    java.awt.Desktop.getDesktop().browse(
                        new java.net.URI("https://blog.naver.com/garpsu/224213400580"));
                } catch (Exception ex) {
        ...
        closeItem.addActionListener(e -> host.disposeInstance());
        ...
        exitItem.addActionListener(e -> host.exitAll());
    ...
    private JMenu buildSlideShowMenu(JMenu slideMenu) {
        ...
        for (int i = 0; i < overlayVals.length; i++) {
            ...
            ovg.add(oi);
            oi.addActionListener(e2 -> { host.setSlideOverlay(ov); host.repaintClock(); });
            overlayMenu.add(oi);
        }
    ...
    private JMenu buildCameraMenuItem() {
        ...
        JMenuItem camStart = new JMenuItem("▶ 시작 / 주소 변경...");
        ...
        JMenuItem camStop = new JMenuItem("⏹ 중지");
...
*/
	...
	*/
    ...
    // ═══════════════════════════════════════════════════════════
    //  HostContext 구현체 (AnalogClockSwing 에서 이동)
    // ═══════════════════════════════════════════════════════════
    public static class ClockHostContext implements HostContext {

        private final AnalogClockSwing app;

        public ClockHostContext(AnalogClockSwing app) {
            this.app = app;
        }
        ...
        @Override public int[]       getIntervals()   { return AnalogClockSwing.INTERVALS; }
        @Override public boolean     checkAutoStart() { return WindowsAutoStart.check(); }
        ...
        @Override public void showChimeDialog()     { app.chimeController.showChimeDialog(); }
        ...
        @Override public void showAlarmDialog()     { app.alarmController.showAlarmDialog(); }
        ...
        @Override public void disposeInstance() {
            ...
            if (app.slideTimer      != null) app.slideTimer.stop();
            app.tg.stopPolling();
            app.startHidden = false;
            app.dispose();
            AnalogClockSwing.instanceCount--;
            if (AnalogClockSwing.instanceCount <= 0) {
        ...
        // ── SlideShow ───────────────────────────────────────
        @Override public boolean isSlideRunning()    { return app.slideTimer != null && app.slideTimer.isRunning() && !app.slideImages.isEmpty(); }
        ...
        @Override public void    openCityInstance(String cn, java.time.ZoneId zone) {
            SwingUtilities.invokeLater(() -> new AnalogClockSwing(app, cn, zone));
        }
        ...
        @Override public void    kakaoLogin()                   { app.kakao.kakaoLogin(); }
